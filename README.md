...
本项目以交互地球为视觉主体，展示个人介绍、项目、CSDN 博客、学习足迹、地图与图表作品及联系方式。支持电脑、平板与手机，使用 Vue、Vite 和 Three.js 实现纯前端网站；无需数据库或后端服务。
...
ResizeObserver 监听容器，更新画布尺寸和相机比例，同时保留相对缩放。1000px 及以上使用双栏布局；小于 1000px 使用上下布局；小于 600px 的选项卡采用三列排列。
...
构建仍有 Three.js 分块超过 500 kB 的提示。地球已经动态加载，但纹理与 GPU 渲染仍有资源开销。本项目没有后台管理、在线留言、自动博客同步或自动论文图表生成。
...
手机上地球位于个人信息上方，取消桌面最小宽度与画布外扩；画布随屏宽调整。选项卡按钮至少 44px 高，长文章和学习足迹使用页面滚动，避免嵌套滚动区域。地图图片随容器缩放，邮箱与页脚可换行。

地球沿用 OrbitControls：单指旋转、双指缩放。画布捕获触摸手势，页面上下滑动需从地球外的区域开始。当前未进行实体手机触摸与 GPU 性能测试。